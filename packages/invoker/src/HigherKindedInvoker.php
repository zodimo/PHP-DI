...
use Invoker\HKT\CallableResolver;
 ...
 * Invoke a callable.
 */
class HigherKindedInvoker implements InvokerInterface
{
    ...
    public function __construct(?ParameterResolver $parameterResolver = null, ?HigherKindedContainerInterface $container = null)
    {
    ...
    public function call($callable, array $parameters = [])
    {
        ...
        if (! is_callable($callable)) {
            throw new NotCallableException(sprintf(
                '%s is not a callable',
                is_object($callable) ? 'Instance of ' . $callable::class : var_export($callable, true)
            ));
        }
